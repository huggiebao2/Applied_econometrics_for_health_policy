...
*Perform PSM (nearest neighbourhood matching)
...
psmatch2 enrolled $controls, out(health_expenditures1) common /*imposes a common support by dropping treatment observations whose pscore is higher than the maximum or less than the minimum pscore of the controls*/
   *Examine new variables created by Stata
sort _id
tab _treated
tab _treated enrolled
tab _support
tab _weight if _treated==1, miss
...
sum health_expenditures0 health_expenditures1 health_exp_match0 health_exp_match1 if enrolled==1
...
drop if matched==0 & enrolled==0 /*drop nonenrolled not matched to enrolled*/
